(function () {
  ...
  var hostAllowed = window.location.hostname === 'aminhanifm.github.io'
    || measurementId.indexOf('G-TEST') === 0;
  var scrollThresholds = [25, 50, 75, 90];
  var engagementMilestones = [15, 30, 60, 120];

  if (!measurementId || !/^G-[A-Z0-9]+$/.test(measurementId)) return;

  window.__aminAnalyticsHostAllowed = hostAllowed;
  if (!hostAllowed) {
  ...
  function isOptedOut() {
    return window.__aminAnalyticsOptedOut === true || getOptOut();
  }
  ...
  window.addEventListener('storage', function (event) {
    ...
    var optedOut = event.newValue === 'true';
    window.__aminAnalyticsOptedOut = optedOut;
    window['ga-disable-' + measurementId] = optedOut;
    document.documentElement.dataset.analytics = optedOut ? 'disabled' : 'enabled';
  });
  ...
  document.addEventListener('click', function (event) {
    ...
    if (link) {
      ...
      if (isDownload) {
        ...
        track('internal_navigation_click', {
          target_path: url.pathname,
          target_anchor: url.hash.slice(1),
        });
